				...
				<div class="container" style="position:relative;">
					<div class="row main_row">
						<?php
							...
							$profile_percentage=getProfilePercentage($profile_user_id);
									...
									<div class="box mb-3 shadow-sm border rounded bg-white profile-box text-center">
										<div class="py-3 px-3 border-bottom">
											...
											<?php
												...
												if($last_designation)
												{
													$page_post_html.='<p style="font-size:12px;margin-top:-5px;"><a href="'.base_url.'u/'.$user_row['username'].'">'.$last_designation.'</a></p>';
												}
												if($last_education)
												{
													$page_post_html.='<p style="font-size:12px;margin-top:-12px;"><a href="'.base_url.'u/'.$user_row['username'].'">'.$last_education.'</a></p>';
												}
												echo $page_post_html;
											?>
										...
										<div class="box-body">
											...
											<div style="width:100%;" id="professional_gallery">
												<div id="professional_data_matrix" style="width:100%;min-height:100px;">
													<?php
														$gquery="SELECT * FROM gallery WHERE is_professional=0 AND delete_status=0 AND is_private=0 AND is_draft=0 AND user_id='".$profile_user_id."' AND type LIKE 'image/%' ORDER BY id DESC LIMIT 6";
														//echo $gquery;
											...
											<div class="" style="width:100%;display:none;" id="personal_gallery">
												<div id="personal_data_matrix" style="width:100%;min-height:100px;">
													<?php
														$gquery="SELECT * FROM gallery WHERE is_professional=1 AND delete_status=0 AND is_private=0 AND is_draft=0 AND user_id='".$profile_user_id."' AND type LIKE 'image/%' ORDER BY id DESC LIMIT 6";
														$gresult=mysqli_query($conn,$gquery);
														if(mysqli_num_rows($gresult)>0)
														{
															while($grow=mysqli_fetch_array($gresult))
											...
											<div class="modal-dialog modal-lg" role="document">
												<div class="modal-content">
													...
													<div class="modal-body">											
														<div class="row">
															<div class="col-lg-12" style="box-shadow: 0 0 0 1px rgba(0,0,0,.15), 0 2px 3px rgba(0,0,0,.2);transition: box-shadow 83ms;background:#fff;padding:10px;margin: 0 0px 10px;border-radius:2px;">
																	...
																	<?php
																		...
																		if(mysqli_num_rows($result)>0)
																		{
																			while($row=mysqli_fetch_array($result))
																			{
																				...
																				$htmlData=$htmlData."<div class='col-lg-6' style='margin-bottom:15px;border:1px solid gray;border-radius:10px;height:30px;max-width:48%;margin-right:1%;'><div class='row' style='margin-top:5px;'>";
																				$htmlData.="<div class='col-lg-7'><h6 style='font-size:14px;' class='text-truncate'>".ucfirst(strtolower($row['title']))."</h6></div>";
																				$htmlData.="<div class='col-lg-3'><h6 style='font-size:12px;'>";
																				...
																				$htmlData.="</h6></div>";
																				$htmlData.="<div class='col-lg-2'><h6><a href='javascript:void(0);' title='Remove' class='remove_skill' onclick='removeSkills(".$row['id'].");' style='text-decoration:none;'><i class='fa fa-trash' style='font-size:16px;color:red;'></i></a></h6></div>";
																				$htmlData.="</div></div>";
																			}
																			$htmlData.="<script>
																		...
																		{
																			$htmlData="<div class='col-lg-12'><h6 style='text-align:center;'>No Skills has been added yet.</h6></div>";
																		}
																		echo $htmlData;
																	?>
																</div>	
						...
						<main class="col col-xl-6 order-xl-2 col-lg-12 order-lg-2 col-lg-12 col-sm-12 col-12">
							<div class="box shadow-sm border rounded bg-white mb-3">
								<div class="box-title border-bottom p-3">
									<h6 class="m-0 font-weight-bold">About <?php echo $user_row['first_name']; ?></h6>
								</div>
								<div class="box-body p-3">
									<?php if($users_personal_row!=false) { echo '<p class="more" id="amazing_about_you_text" style="font-size:14px !important;text-align:justify;">'.trim(filter_var(html_entity_decode($users_personal_row['about']),FILTER_SANITIZE_STRING)).'</p>'; } else { echo '<p id="amazing_about_you_text" class="more" style="font-size:14px !important;text-align:justify;">There is no data to show.</p>'; } ?>
								</div>
							</div>
