<div class="p-3 user_section_jobs">
	...
	<?php
		...
		$people_search_query="SELECT * FROM jobs WHERE (job_title LIKE '%".$search."%' OR job_description LIKE '%".$search."%' OR skills LIKE '%".$search."%' OR job_company LIKE '%".$search."%' OR job_location LIKE '%".$search."%' OR job_title IN('".implode("','",$search_arr)."') OR id IN('".implode("','",$user_search_arr)."')) AND user_id!='".$_COOKIE['uid']."' AND status=1";
		if($search=="")
		{
			$people_search_query="SELECT * FROM jobs WHERE user_id!='".$_COOKIE['uid']."' AND status=1";
		}
