...
<div class="p-3 user_section_bridge">
	...
	<?php
		$friends=array();
		$friends_query="SELECT * FROM user_joins_user WHERE user_id='".$_COOKIE['uid']."' AND status=1";
		...
		$friends_query="SELECT * FROM user_joins_user WHERE r_user_id='".$_COOKIE['uid']."' AND status=1";
		$friends_result=mysqli_query($conn,$friends_query);
		if(mysqli_num_rows($friends_result)>0)
		{
			while($friends_row=mysqli_fetch_array($friends_result))
	...
	<?php
		...
		else
		{
			...
			<div class="col-md-12" style="min-height:100px;">
				<p style="text-align:center;">you have no connections in your bridge.</p>
			</div>
			<?php
		}
	?>
