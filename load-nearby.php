<?php
    ...
    $geoIP[]  = unserialize(file_get_contents("http://www.geoplugin.net/php.gp?ip=$ipAddr"));

?>
<div class="p-3 user_section_nearby">
	<div class="row">
        <div class="col-md-12">
            <h6>Nearby Peoples</h6>
        </div>
	<?php
		//print_r($geoIP);die();
		$friends=array();
		...
		$coordinates=array();
		$lattitude=$geoIP[0]['geoplugin_latitude'];
		$longitude=$geoIP[0]['geoplugin_longitude'];
		
		if($userData['lattitude']!="" && $userData['lattitude']!=null)
		{
			$lattitude=$userData['lattitude'];
		}
		if($userData['longitude']!="" && $userData['longitude']!=null)
		{
			$longitude=$userData['longitude'];
		}
		$coordinates['longitude']=$longitude;
		$coordinates['latitude']=$lattitude;
		$nearby_query=getNearbyUsers($coordinates,$_COOKIE['uid']);
		//echo $nearby_query;
		//$nearby_query="SELECT r_user_id,distance FROM distance_matrix WHERE user_id='".$_COOKIE['uid']."' ORDER BY distance ASC LIMIT 100";
		...
		if(mysqli_num_rows($nearby_result)>0)
		{
			...
			{
				$distance=displayDistance($nearby_row['distance']);
				
				$nearby_user_id=$nearby_row['id'];
				$friends_query="SELECT * FROM user_joins_user WHERE user_id='".$_COOKIE['uid']."' AND r_user_id='$nearby_user_id' AND status=1";
				...
				$friends_query="SELECT * FROM user_joins_user WHERE r_user_id='".$_COOKIE['uid']."' AND user_id='$nearby_user_id' AND status=1";
				$friends_result=mysqli_query($conn,$friends_query);
				if(mysqli_num_rows($friends_result)>0)
				{
					while($friends_row=mysqli_fetch_array($friends_result))
				...
				$bridge_query="SELECT * FROM users WHERE id='$nearby_user_id' AND status=1";
			...
			?>
					 <div class="col col-6 col-lg-3" id="user_section_bridge_<?php echo $connect_user_id; ?>">
						<b href="profile">
						   <div class="border network-list network-item rounded mb-3">
							  <div class="p-3 text-center network-header-custom">
								 ...
								 <div class="font-weight-bold">
									...
									<div class="small text-black-50 user-desc-small"><?php if($last_designation){ echo $last_designation.'<br/>'; } if($last_education){ echo $last_education.'<br/>'; }  if($home_town!="") { echo "Location : ".$home_town."<br/>"; }if($distance!=""){ echo $distance; } ?></div>
								 </div>
								</div>
		...
		else
		{
			?>
			<div class="col-md-12">
				<h5 style="text-align:center;">No nearby connection.</h5>
				<!--<p><?php //echo $nearby_query; ?></p>-->
			</div>
			<?php
		}
...
<script>
	loadImageSlider("user_section_nearby");
</script>
