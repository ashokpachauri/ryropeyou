$(document).ready(function() {
	...
	/*'<a class="gallery-action-asmainh">${captions.setting_asMainH}</a>' +
	'<a class="gallery-action-asmainh">${captions.setting_asMainH}</a>' +*/
	// enable fileuploader plugin
	function loadGallery(gallery_type='personal')
	{
		
		if($('#'+gallery_type).length>0)
		{
			var pre_flag=$("#"+gallery_type).data("pre");
			
			var $fileuploader = $('#'+gallery_type).fileuploader({
				limit: 100,
				fileMaxSize: 100,
				...
				thumbnails: {
					...
					item: '<li class="fileuploader-item">' +
							   ...
							   '</div>' +
						  '</li>',
					item2: '<li class="fileuploader-item file-main-${data.isMain} file-banner-${data.isBanner}">' +
							   '<div class="fileuploader-item-inner">' +
								   '<div class="actions-holder">' +
				...
				upload: {
					url: './php/gallery-ajax.php?type=upload&gallery_type='+gallery_type,
					data: null,
					type: 'POST',
				...
				editor: {
					...
					onSave: function(dataURL, item) {
						...
						if (item.appended && item.data.listProps) {
							...
							$.post('php/gallery-ajax.php?type=resize&gallery_type='+gallery_type, {name: item.name, id: item.data.listProps.id, _editor: JSON.stringify(item.editor)}, function() {
								// update the image
				...
				sorter: {
					onSort: function(list, listEl, parentEl, newInputEl, inputEl) {
						...
						// send request
						$.post('php/gallery-ajax.php?type=sort&gallery_type='+gallery_type, {
					...
					$('body').on('click', function(e) {
						...
						if ($target.is('.gallery-action-rename')) {
							...
							if (x && item.data.listProps) {
								$.post('php/gallery-ajax.php?type=rename&gallery_type='+gallery_type, {name: item.name, id: item.data.listProps.id, title: x}, function(result) {
									try {
						...
						if ($target.is('.gallery-action-asmain') && item.data.listProps) {
							$.post('php/gallery-ajax.php?type=asmain&gallery_type='+gallery_type, {name: item.name, id: item.data.listProps.id}, function() {
						...
						// set banner
						if ($target.is('.gallery-action-asmainh') && item.data.listProps) {
							$.post('php/gallery-ajax.php?type=asmainh&gallery_type='+gallery_type, {name: item.name, id: item.data.listProps.id}, function() {
								api.getFiles().forEach(function(val) {
									delete val.data.isBanner;
									val.html.removeClass('file-main-0 file-main-1');
								});
								item.html.addClass('file-main-1');
								item.data.isBanner = true;
				...
				onRemove: function(item) {
					// send request
					if (item.data.listProps)
						$.post('php/gallery-ajax.php?type=remove&gallery_type='+gallery_type, {
							name: item.name,
							id: item.data.listProps.id
						});
				},
				captions: {
					feedback: 'Drag & Drop',
					setting_asMain: 'Set As Profile',
					setting_asMainH: 'Set As Banner',
					setting_download: 'Download',
					setting_edit: 'Edit',
					setting_open: 'Open in new tab',
			...
			// preload the files
			//console.log(pre_flag);
			
			//console.log(pre_flag);
			if((""+pre_flag)=="1")
			{
				$.post('php/gallery-ajax.php?type=preload&gallery_type='+gallery_type, null, function(result) {
