<?php
	...
	$id=$_REQUEST['id'];
	mysqli_query($conn,"UPDATE users SET google_id='' WHERE google_id='$id'");
	$query="SELECT * FROM users WHERE email='$email' AND status!=3";
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		mysqli_query($conn,"UPDATE users SET google_id='$id'".$temp.",validated=1,status=1 WHERE id='".$row['id']."'");
		$_SESSION['uid']=$row['id'];
	...
	{
		mysqli_query($conn,"UPDATE users SET google_id='' WHERE google_id='$id' AND status=3");
		...
		$query="INSERT INTO users SET first_name='$name',email='$email',google_id='$id',role='$role',status=1,added=NOW(),validated=1";
		if($result=mysqli_query($conn,$query))
		{
			...
			//mysqli_query($conn,"INSERT INTO users_personal SET user_id='".$uid."'");
			echo "SUCCESS";
		}
		else
		{
