...
?>
<div class="p-3 follow_user_data_hub">
	<div class="row">
        <div class="col-md-12">
            <h6>Peoples I Follows</h6>
        </div>
	<?php
		$user_id=$_COOKIE['uid'];
		$response['status']="success";
		$requested_query="SELECT * FROM followers WHERE follower_id='".$user_id."' AND status=1";
		$requested_result=mysqli_query($conn,$requested_query);
		$requested=mysqli_num_rows($requested_result);
		if($requested<=0)
		{
			$data="<div class='col-md-12'><p style='text-align:center;'>You are not following anyone.</p></div>";
		}
		else
		{
			$data="";
			while($requested_row=mysqli_fetch_array($requested_result))
			{
				...
				if(mysqli_num_rows($user_result)>0)
				{
					...
					$data.='<div class="col col-6 col-lg-3" id="user_section_followed_'.$connect_user_id.'">
					<b href="profile">
		...
		$data=$data.'<script>
	loadImageSlider("follow_user_data_hub");
</script>';
        echo $data;
	?>
	</div>
